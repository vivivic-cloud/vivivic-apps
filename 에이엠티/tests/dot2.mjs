...
await p.evaluate((sk)=>{
  ...
  const mk=(i,코드,이름,부속들,끝난것)=>{
    ...
    const parts=부속들.map((nm,j)=>({cKey:코드+'_'+nm,nm,pm:4,dq:50,orderCode:코드,orderKey:'d'+i,orderName:이름,
      plateName:'PB-18T',coating:'양면',finish:'화이트',rw:600+j,rd:400,isCuttingCard:true,sheets:5,perSheet:4,boardParts:[],pRowIndex:null,coveredOrderIds:[1400+i]}));
    부속들.forEach((nm,j)=>{ if(끝난것.includes(j)) comp[코드+'_'+nm]={'재단':true}; });
  ...
  const st={pool:[]}; ['AMT'].forEach(m=>st[m]=[]);
  st['AMT']=parts.map(x=>x.cKey);
...
console.log('  재단 판 점 모양:', JSON.stringify(await p.evaluate(()=>[...document.querySelectorAll('.wo-grp-점 i')].map(e=>e.className||'빈'))));
