...
console.log('■ 갈래 단추:', await p.evaluate(()=>[...document.querySelectorAll('#ams-갈래 .amtb-chip')]
  .map(e=>e.innerText.trim()).join(' · ')));
console.log('■ 알약 줄(접혀 있다):', await p.evaluate(()=>['색','두께','가공']
...
console.log('■ 「색상」 갈래 펼치기:', await 탭('#ams-갈래 .amtb-chip', 0));
