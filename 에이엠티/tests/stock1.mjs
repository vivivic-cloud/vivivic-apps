...
// 안 고른 자리로 돌아가면 목록은 비운다 — 거르지 않은 자재를 늘어놓지 않는다(09-17 02:00)
판('풀면 목록이 비고 머리는 전체 짝 수로 돌아온다',
   푼뒤.줄===0 && /^35짝$/.test((푼뒤.머리||'').trim()), 푼뒤.줄+'줄 · 머리 '+푼뒤.머리);
...
판('같은 알약을 다시 누르면 풀리고 목록이 도로 빈다', 다시1.줄===큰것.짝 && 다시2.줄===0,
   다시1.줄+' → '+다시2.줄);
